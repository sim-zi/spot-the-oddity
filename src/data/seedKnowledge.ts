...
// 카테고리별로 랜덤 지식 가져오기
export function getRandomKnowledge(category?: string): Knowledge {
  let filtered = SEED_KNOWLEDGE;

  if (category && category !== "random") {
    filtered = SEED_KNOWLEDGE.filter((k) => k.category === category);
  }

  if (filtered.length === 0) {
    filtered = SEED_KNOWLEDGE;
  }
